/*
ImageJ script to carry colocalisation analyses of scrambled images.
*/ 
...
for (i = 0; i < toAnalyse.length; i++) {
	if(! File.isDirectory(inputDir + File.separator + toAnalyse[i])){
		analyse_image(inputDir+File.separator +toAnalyse[i],inputDN,outputDir);
	}
}
...
function analyse_image(inputF,inputDN,outputDir) { 
// Analyse image
	...
	run("Split Channels");	
	for (i = 1; i < 3; i++) {
		baseFile = 	outputDir + substring(mainImage,0,mainImage.length-4) +"-1.czi-log.txt";
		thrs = GetThreshFromTxt(baseFile);
		if(i==1) thra = thrs[0];
		else thrb = thrs[1];
...
//	run("Bandpass Filter...", "filter_large="+highLim+" filter_small="+lowLim+" suppress=None tolerance=5 process"); 
	...
	if(ncrop==0){
		...
		saveAs("Tiff", outputDir+mainImage+"-BackSub-Scrambled.tif");
		close(mainImage+"-BackSub-Scrambled.tif");
	}
	else {
		for(i=1;i<=ncrop;i++){
...
function run_Jacop(imageName,outputDir){
	...
	runMacro("C:\\Users\\sandy\\Documents\\Script imageJ\\Scramble.ijm"); // Script to scramble an image
	close("C3-"+imageName);
	for (i = 1; i < 3; i++) {
	...
	selectWindow("Log");
	save(outputDir + imageName +"-log-Scrambled.txt");
	close("Log");
}
...
function GetThreshFromTxt(baseFile){
// Get threshold values from previous results
	print("Thresholds retrieved from used:");
	print(baseFile);
	filestring = File.openAsString(baseFile);
	idx1 = indexOf(filestring, "threshold value of");
	idx2 = indexOf(filestring, "for",idx1+1);
	idx3 = indexOf(filestring, "and",idx2+1);
	idx4 = indexOf(filestring, "for",idx3+1); 
	tra = substring(filestring, idx1+19,idx2);
	trb = substring(filestring, idx3+4,idx4);
	print("Thresholds used: ",tra," and ",trb);
	return newArray(tra,trb);
}
